...
def main():
    ...
    # --- _Run observe->learn->evaluate TRANSITION (the seam the earlier tests missed) --------------
    # A discovered-but-UNMEASURABLE state signal (one cycle within the window -- the real bench case
    # for a slow process at --observe 30) must still proceed to learn + evaluate, not exit after
    # observe (that was the silo regression). Drive the _Run state machine directly, forcing each
    # window's deadline, so the transition logic is exercised without tshark or wall-clock waiting.
    ...
    tbuf = io.StringIO()
    tem = obs.Emitter(enabled=True, out=tbuf)
    src = SimpleNamespace(extractor=OpcUaExtractor())
    args = SimpleNamespace(observe=1.0, learn=1.0, grammar=None, profile=None)
    run = obs._Run([src], args, tem, log=lambda *a: None)

    for e in one_cycle_pubs():
        run.dispatch(src.extractor, e)          # observe-phase collection
    run.observe_end = _time.time() - 1          # force the observe deadline
    run.tick()
    check("unmeasurable signal is still evaluable -> proceeds to learn, does not exit (rc=2)",
          run.phase == "learn" and not run.stop, f"(phase={run.phase}, stop={run.stop})")

    run.dispatch(src.extractor, NormalizedEvent(timestamp=6.0, protocol="OPCUA/binary",
                 op="WRITE_REQUEST", direction="request", target="ns=4;i=23", value=80.0,
                 server=SRV_A, raw={"variant_type": 0x0A}))     # a learn-phase write
    run.learn_end = _time.time() - 1            # force the learn deadline
    run.tick()
    check("after learn the run reaches continuous evaluate", run.phase == "evaluate" and not run.stop,
          f"(phase={run.phase})")
    stages = [json.loads(l)["stage"] for l in tbuf.getvalue().splitlines()
              if json.loads(l).get("type") == "stage"]
    check("stage events announced observe -> learn -> evaluate",
          stages == ["observe", "learn", "evaluate"], f"(={stages})")
